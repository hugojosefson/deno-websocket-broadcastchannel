...
/**
 * TODO: rename and restructure to return something like a `Promise<Server>` or `Promise<Client>` instead of an infinite loop.
 * TODO: perhaps this should be a class instead of a function?
 * TODO: or perhaps this should be a generator function?
 */
async function main(options = DEFAULT_LISTEN_OPTIONS) {
  const listenOptions = { ...DEFAULT_LISTEN_OPTIONS, ...options };
  const log = log0.sub("main");
  while (true) {
    try {
      while (true) {
        await beServer(listenOptions);
        log("Server closed. trying again...");
      }
    } catch (e) {
      if (e.code === "EADDRINUSE") {
        log("Address in use. Let's try being the client instead...");
      } else {
        throw e;
      }
    }
    await connectAsClient(listenOptions);
    log("Client closed. trying again...");
  }
}
